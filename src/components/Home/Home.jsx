...
export default function Home() {
    ...
    return <>
        ...
        <div style={{ 'zIndex': '9999', 'display': 'none' }} className="position-fixed bottom-0 start-0 end-0  alert alert-success text-center " id='successMsg'> Product Added Successfully </div>
                        ...
                        <div className="item .bg-secondary rounded-3 position-relative my-3 text-center">
                            <Link to={`/ProductsDetails/${proooo.id}`}>
                                <div className="upper">
                                    ...
                                    <img src={proooo.imageCover} className='w-100' alt={proooo.tile} />
                                    <h6 className='text-center text-black' >{proooo.title.slice(0, proooo.title.indexOf('', 20))}</h6>
                                    <h6 className='text-black'>{proooo.subcategory.name}</h6>
                                    <h6>price: {proooo.priceAfterDiscount ? <>
                                        <span className='text-decoration-line-through'>{proooo.price}</span>
                                        <span className='ms-3'>{proooo.priceAfterDiscount}</span>
                                    </> : <span>{proooo.price}</span>}</h6>
                                    <div className='  position-absolute text-black top-0 end-0 bg-danger '>
                                        {proooo.ratingsAverage}</div>
                                </div>
                            </Link>


                            <div className="lower">
                                <button id={`addBtn${idx}`} onClick={function () { addMyProduct(proooo.id, idx) }} className='btn btn-danger'> +</button>
